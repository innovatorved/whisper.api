...
You can authenticate the WebSocket in any of these ways (first match wins):

1. Query parameter: `?token=YOUR_API_KEY` (convenient for quick tests; tokens may appear in access logs).
2. Header: `Authorization: Token YOUR_API_KEY` (preferred when your client supports it).
3. Subprotocol list: e.g. `Sec-WebSocket-Protocol: token, YOUR_API_KEY` (some browser Deepgram-style clients use this).
