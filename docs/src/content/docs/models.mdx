...
3. **Register** the model in `app/utils/constant.py` (`model_names`, and optionally `model_info` / `model_urls`) so the API knows the `model` query id and filename.
...
5. **Verify** the model appears (no auth required):
   ```bash
