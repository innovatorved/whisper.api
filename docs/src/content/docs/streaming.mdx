...
<Aside type="note">
  For **PCM** streams, the server fills an internal buffer until roughly `STREAM_CHUNK_DURATION_MS` of audio at your **`sample_rate`** (16-bit mono), then runs transcription. Match `sample_rate` to the audio you send (often `16000` or `48000`).
</Aside>
...
<Aside type="note">
  The `is_final` field indicates whether this is a finalized segment. The `from_finalize` field is reserved for Deepgram compatibility and is currently always `false` in responses.
</Aside>
...
<Aside type="tip">
  Always send a `CloseStream` message before disconnecting so the server transcribes any audio still in the buffer and sends a final `Results` message if applicable.
</Aside>
...
| **Connection rejected** | Verify your API token: query `?token=...`, `Authorization: Token ...`, or subprotocol pairs (see [Authentication](/authentication/)). Unknown `model` closes with code `4002`. |
