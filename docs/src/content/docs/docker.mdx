...
<Aside type="caution">
  Keep `ENABLE_TEST_TOKEN_ENDPOINT=false` in production. The image defaults to `false`; only set it to `true` temporarily on trusted dev machines if you need Swagger token minting.
</Aside>
...
| URL redirects | Optional | `AUDIO_URL_FOLLOW_REDIRECTS` — keep `false` unless you need CDN redirects (see [API reference](/api-reference/)) |
